...
function Hero() {
  ...
  useEffect(() => {
    const fetchHeroImage = async () => {
      // Provide the second type argument as `HeroImageData`
      const { data, error } = await supabase
        .from<HeroImageData>('hero')  // The first type argument is for the table name
        .select('image_path') // The second type argument is for the response type
        .limit(1);
