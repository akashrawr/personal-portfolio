...
// Define types for the portfolio project data using type instead of interface
type Project = {
  id: number;
  title: string;
  description: string;
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      const { data, error } = await supabase
        .from<Project>('portfolio')  // Type for response data
        .select('*')
          ...
          <div className="portfolio-content">
              ...
              {currentProjects.map((project) => (
                <section className="project-card" key={project.id}>
                  <Image
                    ...
                    alt={project.title}
                    width={500} // Set width and height for Next.js Image component
                    height={300}
                  />
                  <h3>{project.title}</h3>
            ...
            <div className="pagination">
              <a
                ...
                className={currentPage === 1 ? 'disabled' : ''} // Add 'disabled' class
              >
              ...
              <a
                href="#!"
                onClick={handleNext}
                className={currentPage === totalPages ? 'disabled' : ''} // Add 'disabled' class
              >
