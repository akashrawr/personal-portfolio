...
function Hero() {
  ...
  useEffect(() => {
    const fetchHeroImage = async () => {
      // Fetching image path from the 'hero' table
      const { data, error } = await supabase
        .from<HeroImageData>('hero')  // Table name is 'hero', and the response will be of type HeroImageData
      ...
      if (error) {
        ...
        // Extract the image path from the first row of the result
